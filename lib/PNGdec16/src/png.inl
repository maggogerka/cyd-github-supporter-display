...
PNG_STATIC void PNGRGB565(PNGDRAW *pDraw, uint16_t *pPixels, int iEndiannes, uint32_t u32Bkgd, int iHasAlpha)
{
    ...
    switch (pDraw->iPixelType) {
        case PNG_PIXEL_GRAY_ALPHA:
            if (u32Bkgd != 0xffffffff) {
                ...
                for (x=0; x<pDraw->iWidth; x++) {
                    c = s[0]; // use the most significant sample byte
                    a = s[pDraw->iBpp == 16 ? 2 : 1];
                    s += pDraw->iBpp == 16 ? 4 : 2;
                    if (a == 0) {
                        usPixel = usBG;
        ...
        case PNG_PIXEL_TRUECOLOR:
            for (x=0; x<pDraw->iWidth; x++) {
                const int sampleStride = pDraw->iBpp == 16 ? 2 : 1;
                usPixel = (s[2 * sampleStride] >> 3); // blue
                usPixel |= ((s[sampleStride] >> 2) << 5); // green
        ...
        case PNG_PIXEL_TRUECOLOR_ALPHA: // truecolor + alpha
            if (u32Bkgd != 0xffffffff) { // user wants to blend it with a background color
                ...
                for (x=0; x<pDraw->iWidth; x++) {
                    const int sampleStride = pDraw->iBpp == 16 ? 2 : 1;
                    r = s[0]; g = s[sampleStride]; b = s[2 * sampleStride];
                    a = s[3 * sampleStride];
                    if (a == 0)
                        usPixel = u16Clr;
                    else if (a == 255) { // fully opaque
